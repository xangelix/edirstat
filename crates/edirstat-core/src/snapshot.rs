...
/// Serializes the snapshot in the Version 3 format to a file on disk.
...
/// Serializes the snapshot in the Version 3 format to an in-memory buffer.
/// If `compress` is enabled, the buffer is wrapped inside a standard Zstandard container.
...
pub fn load_snapshot_from_bytes(
    ...
    if file_bytes.len() >= 4 && file_bytes[0..4] == [0x28, 0xB5, 0x2F, 0xFD] {
        decompressed =
            zstd::decode_all(file_bytes).map_err(|e| crate::EdirstatError::Zstd(e.to_string()))?;
        file_bytes = &decompressed;
    }
